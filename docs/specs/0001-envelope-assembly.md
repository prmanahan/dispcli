...
| `branch` | Required, non-empty, and constrained to this **deliberately enumerated** subset of `git check-ref-format` — the rule is this list, not ref-format parity (amendment 2026-07-18): no control characters or spaces, no `..`, no leading `-`, no trailing `.lock`, and none of `~ ^ : ? * [`. The list targets the **substitution-injection surface**, not ref-name validity in general: `branch` reaches skill/block content verbatim via `{branch}` substitution — downstream of the parse boundary, with no escaping layer — so it is constrained at validation, not escaped at emission. **Accepted v0 hole:** a branch clearing this list can still be rejected by git itself (`feature@{2}`, a trailing `/`, consecutive slashes), surfacing as a downstream `git worktree add` failure rather than a clean `request_invalid` at the boundary. Widening to full ref-format parity is deferred; implementations validate the enumerated list and **must not** add unenumerated clauses, which would over-reject legal branches (`feature/x`, `my.lock.branch`, and brace-bearing values are all legal here). |
